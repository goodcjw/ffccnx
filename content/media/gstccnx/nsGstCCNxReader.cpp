...
nsGstCCNxReader::nsGstCCNxReader(nsBuiltinDecoder* aDecoder)
  ...
  mGstThreadsMonitor("media.gstccnx.threads"),
  mReachedEos(false),
  mByteOffset(0),
  ...
  fpsDen(0)
{
  MOZ_COUNT_CTOR(nsGstCCNxReader);

  mSrcCallbacks.need_data = nsGstCCNxReader::NeedDataCb;
  mSrcCallbacks.enough_data = nsGstCCNxReader::EnoughDataCb;
  mSrcCallbacks.seek_data = nsGstCCNxReader::SeekDataCb;

  mSinkCallbacks.eos = nsGstCCNxReader::EosCb;
  mSinkCallbacks.new_preroll = nsGstCCNxReader::NewPrerollCb;
  mSinkCallbacks.new_buffer = nsGstCCNxReader::NewBufferCb;
...
nsresult nsGstCCNxReader::Init(nsBuiltinDecoderReader* aCloneDonor)
{
  ...
  gst_pad_add_event_probe(sinkpad,
      G_CALLBACK(&nsGstCCNxReader::EventProbeCb), this);
  ...
  g_object_connect(mPlayBin, "signal::source-setup",
      nsGstCCNxReader::PlayBinSourceSetupCb, this, NULL);
...
void nsGstCCNxReader::PlayBinSourceSetupCb(GstElement *aPlayBin,
                                             ...
                                             gpointer aUserData)
{
  nsGstCCNxReader *reader = reinterpret_cast<nsGstCCNxReader*>(aUserData);
...
nsresult nsGstCCNxReader::ReadMetadata(nsVideoInfo* aInfo)
{
  ...
  if (gst_element_query_duration(GST_ELEMENT(mPlayBin),
    ...
    LOG(PR_LOG_DEBUG, ("returning duration %u:%02u:%02u.%09u",
                                         ...
                                         PRInt64 aTimeThreshold)
{
  ...
  while (true)
  {
    ...
    if (timestamp < aTimeThreshold) {
      LOG(PR_LOG_DEBUG, ("skipping frame %u:%02u:%02u.%09u"
            " threshold %u:%02u:%02u.%09u",
                                 ...
                                 PRInt64 aCurrentTime)
{
  ...
  LOG(PR_LOG_DEBUG, ("%p About to seek to %u:%02u:%02u.%09u",
                                        ...
                                        PRInt64 aStartTime)
{
  ...
  if (mDecoder->OnStateMachineThread())
     ...
     * the gstCCNx threads that are actually reading from the resource
     */
    NotifyBytesConsumed();
...
PRInt64 nsGstCCNxReader::QueryDuration()
{
  ...
  if (gst_element_query_duration(GST_ELEMENT(mPlayBin),
    ...
    if (format == GST_FORMAT_TIME) {
      LOG(PR_LOG_DEBUG, ("pipeline duration %u:%02u:%02u.%09u",
            GST_TIME_ARGS (duration)));
      duration = GST_TIME_AS_USECONDS (duration);
    }
  }
